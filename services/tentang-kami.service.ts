...
async function getDataFromFirebase(): Promise<TentangKamiData> {
  try {
...
export async function getTentangKamiData(): Promise<TentangKamiData> {
  try {
    if (typeof window === 'undefined') {
      return await getDataFromFirebase();
    } else {
      const response = await fetch('/api/tentang-kami', {
        method: 'GET',
        headers: {
